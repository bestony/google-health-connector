...
Nitro detects Vercel with no configuration, so the only thing added for deployment is the
migration step:
...
Vercel runs `vercel-build` in preference to `build` when it exists, so leave the project's
**Build Command** on its default — overriding it with `pnpm build` silently skips
migrations. Set `DATABASE_URL` (and `TURSO_AUTH_TOKEN`, `BETTER_AUTH_SECRET`,
`BETTER_AUTH_URL`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`) in **Project Settings →
Environment Variables**; build-time variables are what this step reads.
...
- **Preview deployments migrate whatever they point at.** If previews inherit the
  ...
  Preview environment its own database if that is not what you want.
...
Connections are capped at `MAX_POOL_CONNECTIONS` (5) per process in
`src/db/client.server.ts`. Serverless multiplies that by the number of warm instances; past
a handful, the answer is a pooler in front of the database — PgBouncer, Neon's pooled
endpoint, PlanetScale — not a larger number. libSQL is exempt: it is stateless HTTP.
